...
// Inject glow styles once globally
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Only track pointer on non-touch (desktop) devices for performance
    const mql = window.matchMedia('(hover: hover)');
    if (!mql.matches) return;

    const syncPointer = (e: PointerEvent) => {
      if (cardRef.current) {
        cardRef.current.style.setProperty('--x', e.clientX.toFixed(0));
        cardRef.current.style.setProperty('--xp', (e.clientX / window.innerWidth).toFixed(2));
        cardRef.current.style.setProperty('--y', e.clientY.toFixed(0));
