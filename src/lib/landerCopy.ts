...
// Offers that still have a CampaignLander route. "fun-dive" and "koh-tao" were
// removed 2026-08-28: /koh-tao-diving was retired 2026-07-15 (301 -> /fun-dives)
// and /fun-dives is served by FunDiveLander + funDiveCopy.ts, not this module.
// NOTE: "fun-dive"/"koh-tao" still exist as WhatsAppTopic values (whatsapp.ts
// has its own union) because the live /fun-dives CTAs route on them.
export type Offer = "dsd" | "owd" | "aow" | "sail-rock";
...
const SAIL_ROCK_EN: LanderCopy = {
  ...
  // SAIL_ROCK_DAY: the weekday here must match src/data/sailRockDay.ts.
  departuresLabel: "We sail every Sunday · upcoming departures",
  ...
  dayTimeline: [
    { time: "06:30", label: "Meet at the dive centre - paperwork, gear check, then down to the pier" },
    ...
    { time: "16:00", label: "Back to Koh Tao" },
  ],
...
const SAIL_ROCK_ES: LanderCopy = {
  ...
  // SAIL_ROCK_DAY: the weekday here must match src/data/sailRockDay.ts.
  departuresLabel: "Salimos todos los domingos · próximas salidas",
  ...
  dayTimeline: [
    { time: "06:30", label: "Encuentro en el centro de buceo - papeleo, revisión de equipo y al muelle" },
    ...
    { time: "16:00", label: "Vuelta a Koh Tao" },
  ],
...
const SAIL_ROCK_HE: LanderCopy = {
  ...
  seoDescription:
    "צלילה בסייל רוק - הצלילה הכי טובה במפרץ תאילנד: טיול יום שלם מקוטאו, 3 צלילות כולל שארק איילנד וה\"ארובה\" המפורסמת. 4,000 THB הכל כלול.",
  ...
  // SAIL_ROCK_DAY: the weekday here must match src/data/sailRockDay.ts.
  departuresLabel: "יוצאים כל יום ראשון · יציאות קרובות",
  ...
  dayTimeline: [
    { time: "06:30", label: "התייצבות במועדון - טפסים, בדיקת ציוד וירידה למזח" },
...
> = {
  ...
  "sail-rock": { price: "4000", currency: "THB", duration: "PT9H30M" },
};
