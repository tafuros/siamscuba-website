            ...
            return (
              ...
              >
                {text?.badge && (
                  <span
                    className={`absolute -top-3 ${isRTL ? "right-5" : "left-5"} rounded-full px-3 py-1 text-[11px] font-semibold ${
                      product.recommended ? "bg-ocean-light text-ocean-deep" : "bg-white/15 text-white"
                    }`}
                  >
                    {text.badge}
                  </span>
                )}
                <h3 className="mt-2 font-display text-xl font-semibold">{text?.name}</h3>
                <p className="mt-0.5 text-xs uppercase tracking-wide text-ocean-light/80">{text?.who}</p>
                <p className="mt-2 text-sm text-ocean-surface/75">{text?.meta}</p>

                <ul className="mt-4 flex-1 space-y-1.5">
                  {text?.highlights.map((h, j) => (
                    <li key={j} className="flex items-start gap-2 text-xs text-ocean-surface/75">
                ...
                <div className="mt-5">
                  ...
                  <div className="text-xs text-white/45">{copy.products.perPerson}</div>
                </div>
                <button
